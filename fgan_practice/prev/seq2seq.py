...
class Encoder(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, num_layers, drop_out):
        ...
        self.encoder = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_dim, num_layers=num_layers)
    ...
    def forward(self, x):
        ...
        encoder_output, state_tuple = self.encoder(encoder_embedding, (h_0, c_0))
...
class Decoder(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, num_layers, pred_len, drop_out):
        ...
        # embedding [x, y, z] to [embedding_dim]
        self.embedding = nn.Linear(3, embedding_dim)
        self.decoder = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_dim, num_layers=num_layers)
    ...
    def forward(self, obs_traj_rel, state_tuple):
        ...
        for i in range(self.pred_len):
            ...
            decoder_embedding = torch.unsqueeze(decoder_embedding, 0)                           # increase one dimension (seq_length) for batched input [batch, features] -> [seq_len, batch, features]
