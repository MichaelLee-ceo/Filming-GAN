...
from fgan.utils import check_accuracy, relative_to_abs, get_dset_path, mkdir, plot_traj
...
# Output
parser.add_argument('--output_dir', default=os.path.join(os.getcwd(), 'models'), type=str)
...
def main(args):
    ...
    l2_loss_weights = [1.0]
    for loss_weight in l2_loss_weights:
        args.l2_loss_weight = loss_weight

        pic_path = './pic_result/(sgan)' + 'ehg' + str(args.encoder_h_dim_g) + 'dhg' + str(args.decoder_h_dim_g) + 'ehd' + str(args.encoder_h_dim_d) + 'mlp' + str(args.mlp_dim) + 'n' + str(args.noise_dim) + 'l2' + str(loss_weight) + '/'
        mkdir(pic_path)
        mkdir(args.output_dir)
        
        run = wandb.init(
                        project="social-gan",
                        name =  'ehg' + str(args.encoder_h_dim_g) + 'dhg' + str(args.decoder_h_dim_g) + 'ehd' + str(args.encoder_h_dim_d) + 'n' + str(args.noise_dim) + 'l2' + str(loss_weight),
        ...
        for t in range(args.num_epochs):
            ...
            if True:
                ...
                # logger.info('Done.')
        checkpoint_path = os.path.join(args.output_dir, 'model_%d.pt' % checkpoint['best_t'])
        logger.info('Saving checkpoint to {}'.format(checkpoint_path))
        torch.save(checkpoint, checkpoint_path)
